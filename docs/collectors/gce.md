...
`facts.Gce != nil` signal, if you can read it, you're running on GCE. On non-GCE
hosts the metadata endpoint isn't reachable and the `gce` field drops from
`Facts` silently (no error, no noise).
