...
| -------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Linux    | gopsutil `cpu.InfoWithContext` + `cpu.CountsWithContext` (parses `/proc/cpuinfo`). | [`cpu.rb`](https://github.com/chef/ohai/blob/main/lib/ohai/plugins/cpu.rb) — `lscpu` primary + `/proc/cpuinfo` gap-fill.                                    | **Same secondary source (`/proc/cpuinfo`).** Ohai prefers `lscpu` output and surfaces NUMA topology, per-cache breakdown (L1d/L1i/L2/L3), mitigations/vulnerabilities map, byte order, address sizes, and per-logical-CPU maps — we report gopsutil's aggregate only. |
| macOS    | gopsutil `cpu.InfoWithContext` (sysctl).                                           | Same top-level [`cpu.rb`](https://github.com/chef/ohai/blob/main/lib/ohai/plugins/cpu.rb) — `sysctl hw.ncpu` / `hw.physicalcpu` under the `:darwin` branch. | **Equivalent** on the fields we capture.                                                                                                                                                                                                                              |
