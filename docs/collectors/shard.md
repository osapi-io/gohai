...
| -------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Linux    | `/etc/machine-id` (fallback `/var/lib/dbus/machine-id`) + `os.Hostname`; SHA-256 of the joined string. | [`shard.rb`](https://github.com/chef/ohai/blob/main/lib/ohai/plugins/shard.rb) — concatenates `machinename` + DMI serial + DMI uuid (configurable via `Ohai.config[:plugin][:shard_seed][:sources]`) and hashes with MD5, or SHA-256 under FIPS. | **Equivalent concept, different output shape and inputs.** Ohai's default inputs are `machinename` + DMI serial + DMI uuid (not `machine-id`), and the output is the first 7 hex chars of the digest interpreted as an integer. We use `machine_id` as input and emit the full hex digest — a superset of Ohai's output that consumers can slice to match. |
| macOS    | IOPlatformUUID (via gopsutil host.Info) + `os.Hostname`; SHA-256 of both.                              | Same `shard.rb` — under `:darwin` uses `hardware["serial_number"]` and `hardware["uuid"]`.                                                                                                                                                       | **Equivalent outcome** — stable hardware UUID + machinename; different exact input field names.                                                                                                                                                                                                                                                            |

**Known gaps:** None. The seed format (full hex-encoded SHA-256) is a deliberate
public contract — a superset of Ohai's 7-hex-char integer, so consumers can
slice/modulo freely. We deliberately do not implement Ohai's per-plugin
`[:sources]` configurability.
