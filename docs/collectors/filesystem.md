...
| -------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Linux    | gopsutil `disk.PartitionsWithContext` + `disk.UsageWithContext`. | [`linux/filesystem.rb`](https://github.com/chef/ohai/blob/main/lib/ohai/plugins/linux/filesystem.rb) — combines `df -P`, `df -iP`, `mount`, `lsblk`, `blkid`, and btrfs sysfs. | **Same core data (mounts, capacity, inodes).** Ohai additionally surfaces `uuid` and `label` (via `blkid`/`lsblk`) and btrfs allocation detail. Field-name convention differs (Ohai uses `mount`/`fs_type`; we follow node_exporter). |
| macOS    | gopsutil `disk.PartitionsWithContext` + `disk.UsageWithContext`. | [`darwin/filesystem.rb`](https://github.com/chef/ohai/blob/main/lib/ohai/plugins/darwin/filesystem.rb) — `df`, `mount`, `diskutil`.                                            | **Same core data.** Ohai adds `diskutil`-derived UUID/label on darwin.                                                                                                                                                                |
